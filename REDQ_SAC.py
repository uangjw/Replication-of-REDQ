...
class REDQSACAgent(object):
    ...
    # 返回当前replay buffer的大小，用于确定下一个action根据policy来取还是随机取
    def __get_current_num_data(self):
        return self.replay_buffer.size

    # 对环境explore阶段，根据observation返回一个取样得到的action
    ...
    # 测试policy阶段，根据observation返回一个决策得到的action
    ...
    # 根据obs_tensor与act_tensor返回Q值估计
    # obs_tensor本质是observation按集成Q函数的维度生成，act_tensor同理
    ...
    # 根据agent的Q target模式计算REDQ Q target
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        ...
        with torch.no_grad():
            # 在min模式下，Q-target取为Q值的一个子集中的最小值
            ...
            if self.q_target_mode == 'ave':
                ...
                y_q = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob
            # 在rem模式下，Q target取一个由Q值随机集成得到的值
    ...
    # 训练REDQ agent，在每次收集数据点之后调用
    # 数据过于有限时不做q target网络更新
